...
def train(
    ...
    with tqdm(trainloader, desc=f"Epoch {epoch + 1}/{epochs}") as t:
        for batch_data in t:
            # 平均化ベクトルの取得
            _mean_embedding = embedding_matrix[batch_data["input_ids"]]
            mean_embedding = torch.mean(_mean_embedding, dim=1).to(device)
            label = batch_data["label"].to(device).to(torch.float32)

...
class SemanticClassifier(nn.Module):
    ...
    def __init__(self, embedding_matrix: torch.Tensor, n_classes: int) -> None:
        super(SemanticClassifier, self).__init__()
        self.embedding_matrix = nn.Parameter(embedding_matrix)
        self.n_classes = n_classes
        self.linear1 = nn.Linear(in_features=embedding_matrix.size(1), out_features=1, bias=False)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return self.sigmoid(self.linear1(mean_embedding)).squeeze(1)
...
def main(args) -> None:
    ...
    # 学習前の埋め込み行列を保存
    original_embedding_matrix = embedding_matrix.clone()
    original_embedding_matrix = original_embedding_matrix.to(device)

    # 埋め込み行列をGPUに移動
    embedding_matrix = embedding_matrix.to(device)

    # モデルをGPUに移動
    model = SemanticClassifier(embedding_matrix=embedding_matrix, n_classes=2).to(device)
    ...
    # 学習済モデルの保存
    torch.save(model.state_dict(), "78_model.pth")
    ...
    # 学習前後の埋め込み行列の値が変化しているかを判定
    embedding_changed = not torch.equal(original_embedding_matrix, model.embedding_matrix.data)
    print(f"埋め込み行列が変化したか: {embedding_changed}")
